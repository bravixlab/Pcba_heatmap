...
// ════════════════════════════════════════════
function buildAllBoardsData(){
  return boards
    .filter(b=>b.pcbImgData && b.comps.length)
    .map(b=>({
      boardName:b.name,
...
function publishProject(){
  const valid=buildAllBoardsData();
  if(!valid.length){ toast('⚠ Adicione imagem e componentes em ao menos um board'); return; }
...
async function doPublishWithToken(token){
  ...
  const allBoards=buildAllBoardsData();
  const data={
...
async function doPublish(){
  ...
  closePubModal();
  await doPublishWithToken(token);
}
